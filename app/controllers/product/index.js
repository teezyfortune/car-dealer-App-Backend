...
const { constants: { RESOURCE_CREATE_ERROR,
  FETCH_DATA_ERROR, FETCH_DATA_ERROR_MSG, FETCH_DATA_SUCCESS,
  RESOURCE_CREATE_ERROR_STATUS,
  RESOURCE_UPDATE_FAIL_STATUS,
  RESOURCE_UPDATE_SUCCESS, RESOURCE_UPDATE_FAIL,
  RESOURCE_CREATE_SUCCESS } } = constants;
const { updateProductInfo, fetchAllProduct, fetchProduct, searchProductByName } = ProductServices;
...
class ProductController {
  ...
  static async saveProduct(req, res, next) {
    try {
      ...
      return successResponse(res, RESOURCE_CREATE_SUCCESS('Product'), data);
    } catch (e) {
      const dbError = new DBError({
        status: RESOURCE_CREATE_ERROR_STATUS('PRODUCT'),
        message: e.message });
      Helper.moduleErrLogMessager(dbError);
      next(new ApiError({ message: RESOURCE_CREATE_ERROR('Admin') }));
    }
  }
  ...
  static async updateProduct(req, res, next) {
    try {
      ...
      const dbError = new DBError({
        ...
        message: e.message });
      Helper.moduleErrLogMessager(dbError);
      next(new ApiError({ message: RESOURCE_UPDATE_FAIL('Product') }));
    }
  }
  ...
  static async fetchAllVehicleProduct(req, res, next) {
    try {
      ...
      return successResponse(res, FETCH_DATA_SUCCESS('Products'), product);
    } catch (e) {
      const dbError = new DBError({
  ...
  static async fetchOneVehicleProduct(req, res, next) {
    try {
      ...
      return successResponse(res, FETCH_DATA_SUCCESS('Product'), product);
    } catch (e) {
      const dbError = new DBError({
        status: FETCH_DATA_ERROR('PRODUCT'),
        message: e.message });
